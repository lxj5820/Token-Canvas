...
function isValidNode(node: any): node is NodeData {
  ...
  // 校验可选字符串字段
  if (node.prompt !== undefined && typeof node.prompt !== "string")
    return false;
  if (node.imageSrc !== undefined && typeof node.imageSrc !== "string")
    return false;
  if (node.videoSrc !== undefined && typeof node.videoSrc !== "string")
    return false;
  if (node.audioSrc !== undefined && typeof node.audioSrc !== "string")
    return false;
  if (node.model !== undefined && typeof node.model !== "string") return false;
  // 校验可选数值字段
  if (
    node.count !== undefined &&
    (typeof node.count !== "number" || node.count < 1 || node.count > 10)
  )
    return false;
  return true;
}
...
function isValidConnection(conn: any): conn is Connection {
  ...
  for (const field of REQUIRED_CONNECTION_FIELDS) {
    if (
      typeof conn[field] !== "string" ||
      conn[field].length === 0 ||
      conn[field].length > 100
    )
      return false;
  }
...
export function validateWorkflow(
  ...
  if (rawData.nodes.length > maxNodes) {
    return {
      valid: false,
      data: null,
      errors: [`节点数量超过限制 (${rawData.nodes.length} > ${maxNodes})`],
    };
  }
  ...
  if (rawData.connections.length > maxConnections) {
    return {
      ...
      data: null,
      errors: [
        `连接数量超过限制 (${rawData.connections.length} > ${maxConnections})`,
      ],
    };
  }
  ...
  if (rawData.projectName !== undefined) {
    if (
      ...
      rawData.projectName.length <= 100
    ) {
  ...
  if (validNodes.length === 0 && validConnections.length === 0) {
    return {
      valid: false,
      data: null,
      errors: errors.length > 0 ? errors : ["没有有效的工作流数据"],
    };
  }
