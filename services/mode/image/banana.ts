...
const detectEndpointType = (endpoint: string): EndpointType => {
  if (endpoint.includes("generateContent") || endpoint.includes("predict"))
...
export const generateBananaChatImage = async (
  ...
  logger.log(`[BananaPro] Original Endpoint: ${config.endpoint}`);
  logger.log(`[BananaPro] Original Endpoint Type: ${endpointType}`);
  logger.log(`[BananaPro] Has Input Images: ${hasInputImages}`);
  logger.log(
    `[BananaPro] Needs High Res: ${needsHighRes} (resolution: ${resolution})`,
  );
  logger.log(`[BananaPro] Count: ${count}`);
  ...
  if (
    needsHighRes &&
    (endpointType === "OPENAI_CHAT" || endpointType === "UNKNOWN")
  ) {
    // 构建 Gemini 原生格式的 URL
    ...
    logger.log(
      `[BananaPro] Switched to Gemini Native format for high-res support`,
    );
    logger.log(`[BananaPro] New Endpoint: ${geminiNativeEndpoint}`);
  } else {
    targetUrl = constructUrl(config.baseUrl, config.endpoint);
  }

  logger.log(`[BananaPro] Final Endpoint Type: ${endpointType}`);
  logger.log(`[BananaPro] Final Target URL: ${targetUrl}`);

  const isNativeGemini = endpointType === "GEMINI_NATIVE";

  // 处理多张图片生成
  if (count > 1) {
    logger.log(`[BananaPro] Generating ${count} images`);
...
const generateSingleBananaChatImage = async (
  ...
  if (isNativeGemini) {
    logger.log("[BananaPro] Using Gemini Native format");
    // 重要：Google API 要求 imageSize 必须是大写，如 "1K", "2K", "4K"
    const imageSizeUpperCase = resolution.toUpperCase();
    logger.log(
      `[BananaPro] Resolution: ${resolution}, ImageSize: ${imageSizeUpperCase}, Calculated Size: ${calculatedSize}`,
    );
    ...
    if (hasInputImages) {
      logger.log(
        "[BananaPro] Adding reference image for image-to-image generation",
      );
    ...
    logger.log(
      "[BananaPro] Gemini Native Payload:",
    ...
    for (const candidate of candidates) {
      ...
      for (const part of parts) {
        ...
        if (
          part.text &&
          (part.text.includes("http") || part.text.includes("data:"))
        ) {
  ...
  if (inputImages.length > 0) {
    // 重要：Google API 要求 imageSize 必须是大写，如 "1K", "2K", "4K"
    const imageSizeUpperCase = resolution.toUpperCase();
    logger.log("[BananaPro] Detected input images, using image-to-image mode");
    logger.log("[BananaPro] Target URL:", targetUrl);
    logger.log("[BananaPro] Model ID:", config.modelId);
    logger.log(
      `[BananaPro] Resolution: ${resolution}, ImageSize: ${imageSizeUpperCase}, Size: ${calculatedSize}`,
    );
    logger.log(
      "[BananaPro] Input image preview:",
      inputImages[0].substring(0, 100) + "...",
    );
...
- 输出分辨率：${imageSizeUpperCase} (${calculatedSize})`;
    ...
    logger.log(
      "[BananaPro] Sending image-to-image request with payload structure:",
      {
        model: payload.model,
  ...
  const imageSizeUpperCase = resolution.toUpperCase();
  logger.log(
    `[BananaPro] OpenAI Compatible format - Resolution: ${resolution}, ImageSize: ${imageSizeUpperCase}, Size: ${calculatedSize}`,
  );
...
export const generateBananaEdit = async (
  ...
  if (config.endpoint && !config.endpoint.includes("chat/completions")) {
    ...
    targetUrl = constructUrl(
      config.baseUrl,
      `/api/gemini/nano-banana${endpointSuffix}`,
    );
  }
  ...
  const imageSizeUpperCase = resolution.toUpperCase();
  logger.log(
    `[BananaEdit] Target URL: ${targetUrl}, Has Input: ${hasInput}, ImageSize: ${imageSizeUpperCase}`,
  );
  ...
  if (hasInput) {
    ...
    logger.log(
      `[BananaEdit] Input image length: ${inputImages[0].length} chars`,
    );
  }
  ...
  const queryUrl = constructUrl(
    config.baseUrl,
    `/api/gemini/nano-banana/${taskId}`,
  );

  let attempts = 0;
  while (attempts < 120) {
    await new Promise((r) => setTimeout(r, 3000));
    const check = await fetchThirdParty(queryUrl, "GET", null, config, {
      timeout: 10000,
    });
    const statusRaw =
      check.data?.state ||
    ...
    if (["SUCCESS", "SUCCEEDED", "COMPLETED", "OK", "3"].includes(status)) {
      ...
      throw new Error(
        `Banana Edit failed: ${check.fail_reason || check.error || "Unknown error"}`,
      );
    }
    attempts++;
  }
