...
export const SunoHandler = {
  ...
  generate: async (cfg: ModelConfig, prompt: string, params: any) => {
    ...
    try {
      ...
      let audioUrl =
        res.audio_url || res.audio || res.url || res.result?.audio_url;
      ...
      if (res.choices && res.choices.length > 0) {
        audioUrl =
          res.choices[0].message?.audio_url || res.choices[0].audio?.url;
      }
...
const generateChatAudio = async (
  ...
  let enhancedPrompt = `Generate music: ${prompt}. Duration: ${duration}. Style: ${style}. Generate ${count} variation(s).`;
  if (seed !== -1) {
