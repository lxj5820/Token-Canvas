...
export const generateAlibailianVideo = async (
    ...
    // 使用正确的端点
    const endpoint = config.endpoint && config.endpoint !== '/alibailian/api/v1/services/aigc/video-generation/video-synthesis' 
        ? config.endpoint 
    ...
    const queryEndpoint = config.queryEndpoint || '/alibailian/api/v1/tasks/{task_id}';
    ...
    while (attempts < 120) {
        ...
        try {
            ...
            console.log(`[Alibailian] Poll #${attempts + 1}, Status: "${status}", Response:`, JSON.stringify(check, null, 2));

            if (status === 'SUCCEEDED') {
                if (check.output?.video_url) {
                    console.log('[Alibailian] Video completed! URL:', check.output.video_url);
                    return check.output.video_url;
                }
                throw new Error("Alibailian task succeeded but no video_url found.");
            } else if (status === 'FAILED') {
                const errorMessage = check.output?.message || check.message || 'Unknown error';
