...
const retrieveMinimaxFile = async (
  fileId: string,
  taskId: string,
  config: ModelConfig,
): Promise<string> => {
  ...
  while (attempts < 120) {
    ...
    try {
      ...
      if ((flatStatus === "SUCCESS" || flatStatus === "OK") && check.file_id) {
        ...
        throw new Error(
          `Minimax failed: ${check.base_resp?.status_msg || "Unknown"}`,
        );
      }
    } catch (e: any) {
