...
export const generateKlingO1Video = async (
  ...
  if (!taskId) {
    throw new Error(
      `No Task ID returned from Kling O1: ${JSON.stringify(res)}`,
    );
  }
  ...
  while (attempts < 120) {
    ...
    try {
      ...
      const innerStatus =
        check.data?.data?.task_status ||
        check.data?.task_status ||
        check.task_result?.task_status;
      ...
      if (["succeed", "success", "completed"].includes(status)) {
        ...
        const videos =
          check.data?.data?.task_result?.videos ||
          check.data?.task_result?.videos ||
          check.task_result?.videos;
        if (videos && videos[0]?.url) return videos[0].url;

        const images =
          check.data?.data?.task_result?.images ||
          check.data?.task_result?.images ||
          check.task_result?.images;
        ...
        const msg =
          check.data?.data?.task_status_msg ||
          check.task_status_msg ||
          check.fail_reason ||
          "Unknown error";
...
export const generateKlingStandardVideo = async (
  ...
  const taskId =
    res.data?.data?.task_id || res.data?.task_id || res.task_id || res.id;
  if (!taskId) {
  ...
  while (attempts < 120) {
    ...
    try {
      const check = await fetchThirdParty(currentUrl, "GET", null, config, {
        timeout: 10000,
      });
