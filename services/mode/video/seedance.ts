...
export const generateSeedanceVideo = async (
  ...
  while (attempts < 120) {
    ...
    try {
      const check = await fetchThirdParty(qUrl, "GET", null, config, {
        timeout: 10000,
      });
      ...
      if (["completed", "succeeded", "success"].includes(status)) {
        ...
        // 如果URL缺失，记录完整响应以帮助调试
        console.error(
          "Doubao video generation succeeded but URL is missing. Response:",
          check,
        );
        throw new Error(
          "Video generation completed but no video_url found in response",
        );
