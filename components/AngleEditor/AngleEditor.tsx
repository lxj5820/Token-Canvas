...
export const AngleEditor: React.FC<AngleEditorProps> = ({
  ...
  const handleSceneMouseDown = useCallback((e: React.MouseEvent) => {
    ...
    setDragH(hAngle);
    setDragV(vAngle);
    dragStartRef.current = { x: e.clientX, y: e.clientY, h: hAngle, v: vAngle };
  }, [hAngle, vAngle]);
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      const newV = Math.max(-60, Math.min(60, dragStartRef.current.v - dy * 0.3));
      setDragH(newH);
      setDragV(newV);
    };
  ...
  const handleSliderChange = useCallback((type: 'h' | 'v' | 'z', value: number) => {
  ...
  const handleGenerateClick = useCallback(() => {
    onGenerate({ horizontalAngle: hAngle, verticalAngle: vAngle, zoom, includePrompt, count });
  ...
  return (
    ...
    >
      ...
      <div className={`flex items-center justify-between px-4 py-3 border-b ${isDark ? 'border-zinc-700/50' : 'border-gray-200'}`}>
        <h1 className={`text-[15px] font-semibold ${isDark ? 'text-zinc-200' : 'text-gray-900'}`}>多角度编辑器</h1>
        <button
      ...
      {/* ===== 3D Scene ===== */}
      <div className={`relative border-b ${isDark ? 'bg-[#0d0d0d] border-zinc-700/50' : 'bg-gray-100 border-gray-200'}`} style={{ height: 280 }}>
        {/* Scene container */}
        <div
          ref={sceneRef}
          className="absolute inset-0 flex items-center justify-center"
          style={{ perspective: PERSPECTIVE, cursor: isDragging ? 'grabbing' : 'grab' }}
      ...
      <div className={`px-4 py-3 flex flex-col gap-3 border-b ${isDark ? 'border-zinc-700/50' : 'border-gray-200'}`}>
        ...
        <div className="flex items-center gap-3">
          ...
          />
          <span className={`text-[12px] w-10 text-right tabular-nums font-medium ${isDark ? 'text-zinc-200' : 'text-gray-900'}`}>{hAngle}°</span>
        </div>

        {/* 垂直俯仰 */}
        <div className="flex items-center gap-3">
          <span className={`text-[12px] w-16 shrink-0 ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}>垂直俯仰</span>
          <input
            type="range"
            min={-30}
            max={60}
          ...
          />
          <span className={`text-[12px] w-10 text-right tabular-nums font-medium ${isDark ? 'text-zinc-200' : 'text-gray-900'}`}>{vAngle}°</span>
        </div>

        {/* 景别缩放 */}
        <div className="flex items-center gap-3">
          <span className={`text-[12px] w-16 shrink-0 ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}>景别缩放</span>
          <input
            type="range"
            min={0}
            max={15}
        ...
        <div className={`px-3 py-2 rounded-lg text-[11px] border ${
          ...
          从{H_LABELS[hAngle] || `${hAngle}°`}{getVLabel(vAngle)}的角度重新创作，{ZOOM_LABELS[zoom] || '全景'}视角
          {includePrompt && prompt ? `。原始描述：${prompt.slice(0, 50)}${prompt.length > 50 ? '...' : ''}` : ''}
        </div>
      </div>
