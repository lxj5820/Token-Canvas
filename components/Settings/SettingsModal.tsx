...
export const SettingsModal: React.FC<SettingsModalProps> = ({
  ...
  const handleDeleteModel = (key: string) => {
    ...
    if (
      confirm(
        `确定要删除模型 "${modelName}" 吗？删除后将不再显示在模型选择列表中。`,
      )
    ) {
      deleteModel(key);
      setConfigs((prev) => {
    ...
    const exportData = {
      ...
      configs: Object.fromEntries(
        Object.entries(configs).filter(
          ([_, v]) =>
            (v as ModelConfig).key ||
            (v as ModelConfig).baseUrl ||
            (v as ModelConfig).modelId,
        ),
      ),
    };
  ...
  return (
    ...
    >
      ...
      >
          ...
          <div className="p-6 space-y-6">
            ...
            >
              <div className="flex items-start gap-3">
                <div
                  className={`shrink-0 w-8 h-8 rounded-lg flex items-center justify-center ${isDark ? "bg-amber-500/20 text-amber-400" : "bg-amber-100 text-amber-600"}`}
                >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  >
                    <p
                      className={`text-xs ${isDark ? "text-emerald-400" : "text-emerald-700"}`}
                    >
                  ...
                  <p
                    className={`text-[10px] mt-2 ${isDark ? "text-gray-500" : "text-gray-400"}`}
                  >
                    💡 提示：不同中转商的接口参数格式可能不同，如不兼容请参考
                    README 或用 AI 编辑器调整代码
                  </p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <label
                  className={`text-xs font-medium uppercase tracking-wider ${textSub}`}
                >
            ...
            <div className="flex items-center gap-3">
              ...
              <div
                className={`flex p-1 rounded-xl border ${borderColor} ${isDark ? "bg-[#0f0f11]" : "bg-gray-50"}`}
              >
              ...
              {filteredModels.map((key) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <span className={`font-semibold ${textMain}`}>
                          ...
                          {isCustom && (
                            <span
                              className={`ml-2 text-[10px] font-normal ${textMuted}`}
                            >
                              (自定义)
                            </span>
                          )}
                    ...
                    {isExpanded && (
                      <div
                        className={`px-5 pb-5 pt-2 space-y-4 border-t ${borderColor} animate-in slide-in-from-top-2 duration-200`}
                      >
                        {/* MODEL ID */}
                        <div className="flex items-center gap-4">
                          <label
                            className={`w-24 text-xs font-medium uppercase ${textSub} shrink-0 text-right`}
                          >
                            MODEL ID
                          </label>
                          <input
                            ...
                            onChange={(e) =>
                              updateConfig(key, "modelId", e.target.value)
                            }
                        ...
                        <div className="flex items-center gap-4">
                          ...
                          <input
                            ...
                            onChange={(e) =>
                              updateConfig(key, "endpoint", e.target.value)
                            }
                            className={`flex-1 px-4 py-2.5 rounded-xl text-sm border ${borderColor} ${inputBg} ${textMain} outline-none focus:ring-2 focus:ring-yellow-500/20 focus:border-yellow-500/50 transition-all`}
                            placeholder={
        ...
        {/* Footer */}
        <div
          className={`px-6 py-4 border-t ${borderColor} flex items-center justify-between shrink-0`}
        >
          <div className="flex items-center gap-2">
            <button
          ...
          <div className={`text-xs ${textMuted}`}>
            已配置 {Object.keys(configs).filter((k) => isConfigured(k)).length}{" "}
            / {Object.keys(MODEL_REGISTRY).length} 个模型
          </div>
        </div>
      </div>
      ...
      {showAddModel && (
        ...
        >
          <div
            ...
            onClick={(e) => e.stopPropagation()}
          >
            <h3 className={`text-lg font-bold mb-6 ${textMain}`}>
              添加自定义模型
            </h3>

            <div className="space-y-4">
              <div className="space-y-2">
                <label className={`text-xs font-medium uppercase ${textSub}`}>
                  模型名称
                </label>
                <input
                  type="text"
