...
const getProvider = (key: string, name: string): string => {
    ...
    // ByteDance (Jimeng, Doubao, Seedance)
    if (lowerKey.includes('doubao') || lowerKey.includes('seedance') || lowerKey.includes('jimeng') || lowerKey.includes('即梦') || lowerName.includes('jimeng') || lowerName.includes('doubao') || lowerName.includes('seedance')) return 'ByteDance';
...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose, isDark }) => {
    ...
    return (
        <div className="fixed inset-0 z-[250] flex items-center justify-center p-6 bg-black/60 backdrop-blur-sm animate-in fade-in duration-200" onClick={onClose}>
            <div className={`w-full max-w-6xl h-[85vh] rounded-2xl overflow-hidden flex shadow-2xl border ${isDark ? 'bg-[#0B0C0E] border-zinc-800' : 'bg-white border-gray-200'}`} onClick={(e) => e.stopPropagation()}>
                
                {/* Sidebar */}
                <div className={`w-64 flex flex-col border-r ${sidebarBg}`}>
                    <div className="p-6 pb-4 border-b border-dashed border-gray-500/10">
                    ...
                    <div className="flex-1 overflow-y-auto py-6 px-4 space-y-2 custom-scrollbar">
                        <div className={`px-2 py-2 text-[10px] font-bold text-gray-400 uppercase tracking-wider mb-2`}>Service Providers <span className="ml-1 opacity-50">{providers.length}</span></div>
                    ...
                    {/* Footer Actions */}
                    <div className={`p-4 border-t mt-auto ${isDark ? 'border-zinc-800' : 'border-gray-200'}`}>
                ...
                <div className={`flex-1 flex flex-col min-w-0 ${baseBg}`}>
                    {isCreating ? renderCreationDialog() : editingModelKey ? (
                        <div className="flex-1 p-8 overflow-hidden">{renderConfigForm()}</div>
                    ) : (
                        <>
                            {/* Header */}
                            <div className={`px-8 py-6 border-b flex items-center justify-between ${borderMain}`}>
                                <div className="flex items-center gap-3">
                                    <h1 className={`text-xl font-bold ${textMain}`}>{selectedProvider}</h1>
                            ...
                            <div key={selectedProvider} className="flex-1 overflow-y-auto p-8 custom-scrollbar animate-in fade-in slide-in-from-bottom-4 duration-300">
