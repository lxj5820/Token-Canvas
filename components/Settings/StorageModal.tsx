...
export const StorageModal: React.FC<StorageModalProps> = ({
  ...
  const contentBase =
    "fixed left-[50%] top-[50%] z-[250] w-full max-w-lg translate-x-[-50%] translate-y-[-50%] gap-4 border p-0 shadow-lg duration-200 sm:rounded-3xl overflow-hidden max-h-[90vh] flex flex-col";
  ...
  const buttonBase =
    "inline-flex items-center justify-center whitespace-nowrap rounded-xl text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 h-9 px-4 py-2";
  ...
  const renderStorageTab = () => (
    <div className="space-y-4">
      {/* 下载目录设置 */}
      <div
        className={`p-4 rounded-lg border ${borderColor} ${isDark ? "bg-zinc-900/30" : "bg-zinc-50/50"}`}
      >
        <div className="flex items-start justify-between mb-4">
          <div className="flex items-center gap-3">
            <div
              className={`p-2 rounded-md ${isDark ? "bg-zinc-800" : "bg-zinc-100"}`}
            >
              <Icons.FolderOpen
                size={16}
        ...
        <div
          className={`p-2.5 rounded-md ${isDark ? "bg-zinc-950" : "bg-white"} border ${borderColor} mb-4`}
        >
          <div className="flex items-center gap-2">
            <Icons.Folder size={14} className={textMuted} />
            <span
              className={`text-xs font-mono truncate ${storageDir ? textMain : textMuted}`}
            >
        ...
        {!storageService.isFileSystemAccessSupported() && (
          <div
            className={`mt-3 p-2.5 rounded-md text-xs border ${isDark ? "bg-amber-950/20 border-amber-900/30 text-amber-500" : "bg-amber-50 border-amber-100 text-amber-600"}`}
          >
            <div className="flex items-center gap-2">
              <Icons.AlertCircle size={14} />
              <span>
                您的浏览器不支持文件系统访问API，文件将使用默认下载方式
              </span>
            </div>
          </div>
        )}
        ...
        {storageStats && (
          <div className="space-y-4">
            ...
            <div>
              <div className="flex justify-between text-xs mb-2">
                <span className={textMain}>
                  {formatBytes(storageStats.totalUsage)}
                </span>
                <span className={textMuted}>
                  {formatBytes(storageStats.indexedDBQuota)} 配额
                </span>
              </div>
              <div
                className={`h-2 rounded-full overflow-hidden ${isDark ? "bg-zinc-800" : "bg-zinc-200"}`}
              >
                <div
                  className={`h-full rounded-full transition-all duration-500 ${isDark ? "bg-zinc-50" : "bg-zinc-900"}`}
                  style={{
                    width: `${Math.min(100, parseFloat(formatPercent(storageStats.totalUsage, storageStats.indexedDBQuota)))}%`,
                  }}
                />
              </div>
              <p className={`text-[10px] mt-1.5 ${textMuted} text-right`}>
                已使用{" "}
                {formatPercent(
                  storageStats.totalUsage,
                  storageStats.indexedDBQuota,
                )}
                %
              </p>
            </div>
            ...
            <div className="grid grid-cols-3 gap-2">
              <div
                className={`p-3 rounded-md ${isDark ? "bg-zinc-950" : "bg-white"} border ${borderColor}`}
              >
                <p
                ...
                <p className={`text-sm font-medium mt-1 ${textMain}`}>
                  {formatBytes(storageStats.indexedDBUsage)}
                </p>
              </div>
              <div
                className={`p-3 rounded-md ${isDark ? "bg-zinc-950" : "bg-white"} border ${borderColor}`}
              >
                ...
                <p className={`text-sm font-medium mt-1 ${textMain}`}>
                  {formatBytes(storageStats.cacheAPIUsage)}
                </p>
              </div>
              <div
                className={`p-3 rounded-md ${isDark ? "bg-zinc-950" : "bg-white"} border ${borderColor}`}
              >
                ...
                <p className={`text-sm font-medium mt-1 ${textMain}`}>
                  {formatBytes(storageStats.localStorageUsage)}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="space-y-2 mb-4">
            {Object.entries(cacheStats.byType).map(([type, size]) => (
              ...
              >
                <div className="flex items-center gap-2">
                  {type === "image" && (
                    <Icons.Image size={14} className={textSub} />
                  )}
                  {type === "video" && (
                    <Icons.Video size={14} className={textSub} />
                  )}
                  {type === "workflow" && (
                    <Icons.Folder size={14} className={textSub} />
                  )}
                  {type === "other" && (
                    <Icons.Database size={14} className={textSub} />
                  )}
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between">
              ...
              <button
                title="切换缓存状态"
                onClick={() =>
                  handleUpdateSetting("cacheEnabled", !settings.cacheEnabled)
                }
              ...
              >
                <div
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${settings.cacheEnabled ? "translate-x-4.5" : "translate-x-0.5"}`}
                />
              </button>
            </div>
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`text-xs font-mono ${textSub}`}>
                  {formatBytes(settings.maxCacheSize)}
                </span>
              </div>
              <input
                title="调整最大缓存大小"
                ...
                onChange={(e) =>
                  handleUpdateSetting("maxCacheSize", parseInt(e.target.value))
                }
              ...
              />
              <div
                className={`flex justify-between text-[10px] mt-1 ${textMuted}`}
              >
                <span>100 MB</span>
                <span>2 GB</span>
              </div>
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center justify-between">
              ...
              >
                <div
                  className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${settings.autoSaveWorkflow ? "translate-x-4.5" : "translate-x-0.5"}`}
                />
              </button>
            </div>
          </div>
          ...
          {/* 重置设置 */}
          <div
            className={`p-4 rounded-lg border ${isDark ? "border-red-900/30 bg-red-950/10" : "border-red-200 bg-red-50"}`}
          >
            <div className="flex items-center gap-3 mb-4">
              <div
                className={`p-2 rounded-md ${isDark ? "bg-red-950/30" : "bg-red-100"}`}
              >
              ...
              <div>
                ...
                <p
                  className={`text-xs mt-0.5 ${isDark ? "text-red-400/70" : "text-red-500"}`}
                >
                  重置所有设置到默认值
                </p>
              </div>
  ...
  return (
    <div className={overlayBase} onClick={onClose}>
      ...
      >
        {/* Header */}
        <div
          className={`px-6 py-4 border-b ${borderColor} flex items-center justify-between shrink-0`}
        >
          <div className="flex flex-col gap-1">
            <h2
              className={`text-lg font-semibold leading-none tracking-tight ${textMain}`}
            >
              本地存储管理
            </h2>
          ...
          <button
            onClick={onClose}
            className={`rounded-sm opacity-70 ring-offset-background transition-opacity hover:opacity-100 focus:outline-none focus:ring-2 focus:ring-ring focus:ring-offset-2 disabled:pointer-events-none data-[state=open]:bg-accent data-[state=open]:text-muted-foreground ${isDark ? "text-zinc-400" : "text-zinc-500"}`}
          >
            <Icons.X size={16} />
