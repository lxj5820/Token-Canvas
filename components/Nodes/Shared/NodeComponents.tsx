...
export const ThumbnailItem = memo(
  ({ src, index, isDark }: { src: string; index: number; isDark: boolean }) => {
    ...
    return (
      <div
        className={`relative w-[48px] h-[48px] flex-shrink-0 border rounded-lg overflow-hidden shadow-sm group/thumb cursor-pointer hover:border-yellow-500/50 transition-colors ${isDark ? "border-zinc-700 bg-black/40" : "border-gray-300 bg-gray-100"}`}
      >
        <div
...
export const InputThumbnails = memo(
  ({
    inputs,
    ready,
    isDark,
...
export const CustomDropdown = ({
  ...
  const optionHover = isDark
    ? "hover:bg-zinc-800 hover:text-gray-200"
    ...
    <div className="relative h-full flex items-center" ref={ref}>
      ...
      >
        {Icon && (
          <Icon
            size={13}
            className={`transition-colors ${isOpen ? "text-yellow-400" : iconColor}`}
          />
        )}
        <span
          className={`text-[11px] font-medium transition-colors select-none ${isOpen ? (isDark ? "text-gray-200" : "text-gray-900") : isDark ? "text-zinc-400 group-hover:text-zinc-200" : "text-gray-500 group-hover:text-gray-700"} ${Icon ? "min-w-[16px] text-center" : "max-w-[70px] truncate"}`}
        >
          {value}
        </span>
        {!Icon && (
          <Icons.ChevronRight
            size={10}
            className={`transition-all duration-200 ${isOpen ? "rotate-[-90deg] text-yellow-400" : `rotate-90 ${isDark ? "text-zinc-600 group-hover:text-zinc-400" : "text-gray-400 group-hover:text-gray-600"}`}`}
          />
        )}
      </div>
      {isOpen && (
        ...
        >
          {options.map((opt: string) => {
            ...
            return (
              <div
                key={opt}
                className={`px-3 py-1.5 text-[11px] font-medium transition-colors flex items-center justify-between group/item ${isDisabled ? "text-zinc-600 cursor-not-allowed opacity-50" : opt == value ? activeItem + " cursor-pointer" : (isDark ? "text-zinc-400" : "text-gray-500") + ` ${optionHover} cursor-pointer`}`}
                onClick={(e) => {
                  e.stopPropagation();
                  if (!isDisabled) {
                    onChange(opt);
...
export const EditableTitle: React.FC<{
  ...
  const displayBg = isDark
    ? "text-gray-300 hover:border-zinc-700 bg-[#1A1D21]/50"
    : "text-gray-700 hover:border-gray-300 bg-white/50";
  ...
  return isEditing ? (
    <input
      ...
      className={`${inputBg} border rounded px-2 py-0.5 outline-none w-[140px] text-xs font-bold`}
      onClick={(e) => e.stopPropagation()}
      onMouseDown={(e) => e.stopPropagation()}
    />
  ) : (
    <div
      className={`${displayBg} font-bold text-xs px-2 py-0.5 rounded cursor-text border border-transparent truncate max-w-[140px]`}
      onDoubleClick={(e) => {
        e.stopPropagation();
        setIsEditing(true);
        setEditValue(title);
