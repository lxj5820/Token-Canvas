...
const videoToolbarItems = [{ id: 'plot', label: '剧情', icon: Icons.BookOpen }, { id: 'start_end', label: '首尾', icon: Icons.ArrowRightLeft }, { id: 'region', label: '局部', icon: Icons.Scan }, { id: 'camera', label: '机位', icon: Icons.Camera }, { id: 'role', label: '角色', icon: Icons.User }];
        ...
        {isSelectedAndStable && showControls && (
          <div className="absolute top-full left-1/2 -translate-x-1/2 w-full min-w-[450px] pt-3 z-[70] pointer-events-auto" onMouseDown={(e) => e.stopPropagation()}>
              ...
              <div className={`${controlPanelBg} rounded-2xl p-3 shadow-2xl flex flex-col gap-3 border`}>
                  <textarea className={`w-full border rounded-xl p-3 text-[10px] leading-relaxed resize-none focus:outline-none min-h-[70px] no-scrollbar ${inputBg}`} placeholder="Describe the video scene..." value={data.prompt || ''} onChange={(e) => updateData(data.id, { prompt: e.target.value })} onWheel={(e) => e.stopPropagation()} />
