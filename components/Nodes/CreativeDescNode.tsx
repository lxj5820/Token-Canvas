    ...
    return (
        <>
          ...
          <div className={`w-full h-full border rounded-xl p-4 flex flex-col shadow-lg transition-shadow duration-300 ${containerBg} ${containerBorder}`}>
              <div className={`flex items-center gap-2 text-xs mb-3 ${isDark ? 'text-zinc-400' : 'text-gray-500'}`}><div className="p-1.5 bg-yellow-500/10 rounded-md"><Icons.Wand2 size={14} className="text-yellow-500"/></div><span className="font-medium tracking-wide text-[12px]">创意助手</span></div>
              ...
              <button onClick={() => onGenerate(data.id)} className="mt-3 w-full bg-yellow-600 hover:bg-yellow-500 text-white text-xs font-medium py-2 rounded-lg flex items-center justify-center gap-2 transition-all shadow-sm active:scale-[0.98]">
