...
export const CreativeDescNode: React.FC<CreativeDescNodeProps> = ({
  ...
  const controlPanelBg = isDark
    ? "bg-[#1e1e1e] border-zinc-700/80"
    : "bg-white border-gray-200";
  // 根据主题模式设置输入框样式
  const inputBg = isDark
    ? "bg-zinc-900/50 hover:bg-zinc-900 border-transparent focus:border-yellow-500/50 text-zinc-200 placeholder-zinc-500"
    : "bg-gray-50 hover:bg-gray-100 border-gray-200 focus:border-yellow-500 text-gray-900 placeholder-gray-400";
  ...
  return (
    <>
      ...
      {/* 节点容器 */}
      <div
        className={`w-full h-full border rounded-xl p-4 flex flex-col shadow-lg transition-shadow duration-300 ${containerBg} ${containerBorder}`}
      >
        {/* 创意助手标签 */}
        <div
          className={`flex items-center gap-2 text-xs mb-3 ${isDark ? "text-zinc-400" : "text-gray-500"}`}
        >
          <div className="p-1.5 bg-yellow-500/10 rounded-md">
            <Icons.Wand2 size={14} className="text-yellow-500" />
          </div>
          <span className="font-medium tracking-wide text-[12px]">
            创意助手
          </span>
        </div>
        ...
        {data.optimizedPrompt && isSelectedAndStable && showControls && (
          <div
            className={`absolute top-full left-0 w-full mt-3 border rounded-xl p-4 text-xs z-[70] shadow-2xl animate-in slide-in-from-top-2 duration-200 ${controlPanelBg} ${isDark ? "text-gray-300" : "text-gray-700"}`}
          >
