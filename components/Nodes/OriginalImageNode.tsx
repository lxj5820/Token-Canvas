...
export const OriginalImageNode: React.FC<OriginalImageNodeProps> = ({
    data, updateData, onMaximize, onDownload, onDelete, onUpload, isDark = true, selected, showControls, onGridSplitCreateNodes, onLightGenerate, onAngleGenerate
}) => {
    ...
    // - 非标注模式 + 无烘焙图 → 显示原图
    const displaySrc = isAnnotating || isGridSplitting || isLightEditing || isAngleEditing
          ...
          <div className={`w-full h-full relative group rounded-xl border ${containerBorder} ${containerBg} ${data.isStackOpen || isAnnotating || isGridSplitting || isLightEditing || isAngleEditing || (hasResult && isSelectedAndStable && showControls) ? 'overflow-visible' : 'overflow-hidden'} shadow-lg transition-all duration-200`}>
              ...
              {hasResult ? (
                  <>
                      ...
                      {!isAnnotating && !isGridSplitting && !isLightEditing && !isAngleEditing && !data.annotatedImageSrc && (data.annotations?.length || 0) > 0 && (
                          <AnnotationRenderer
