...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const SidebarButton = ({
    ...
    return (
      ...
      >
        <Icon size={20} />
        <div
          className={`absolute left-full ml-3 px-2.5 py-1.5 rounded-lg text-xs font-medium whitespace-nowrap opacity-0 group-hover:opacity-100 transition-all pointer-events-none z-50 ${
            isDark
        ...
        >
          ...
          {shortcut && (
            <span
              className={`ml-2 px-1.5 py-0.5 rounded text-[10px] ${isDark ? "bg-zinc-800" : "bg-gray-100"}`}
            >
              {shortcut}
            </span>
          )}
  ...
  const renderToolsPanel = () => (
    <div className="space-y-3">
      ...
      >
        <div
          className={`w-9 h-9 rounded-lg flex items-center justify-center ${isDark ? "bg-yellow-500/10 text-yellow-400" : "bg-yellow-50 text-yellow-600"}`}
        >
      ...
      >
        <div
          className={`w-9 h-9 rounded-lg flex items-center justify-center ${isDark ? "bg-emerald-500/10 text-emerald-400" : "bg-emerald-50 text-emerald-600"}`}
        >
  ...
  const renderAddPanel = () => {
    const NodeButton = ({
      ...
      >
        ...
        <Icons.ChevronRight
          size={16}
          className={`${textMuted} group-hover:translate-x-0.5 transition-transform`}
        />
      </button>
    );
  ...
  const renderPanel = () => {
    ...
    if (activePanel === "HISTORY") {
      return (
        ...
        >
          {/* Header */}
          <div
            className={`px-5 py-4 border-b ${borderColor} flex items-center justify-between shrink-0`}
          >
            <div className="flex items-center gap-3">
              <div
                className={`p-2 rounded-lg ${isDark ? "bg-yellow-500/10" : "bg-yellow-50"}`}
              >
                <Icons.Clock
                  size={18}
          ...
          <div className={`px-4 pt-4 shrink-0`}>
            <div
              className={`flex p-1 rounded-xl ${isDark ? "bg-zinc-900" : "bg-gray-100"}`}
            >
              <button
          ...
          <div className="flex-1 p-4 overflow-y-auto custom-scrollbar">
            {(historyTab === "image" ? imageNodes : videoNodes).length === 0 ? (
              ...
              >
                <div
                  className={`w-16 h-16 rounded-2xl flex items-center justify-center mb-4 ${isDark ? "bg-zinc-800" : "bg-gray-100"}`}
                >
                  {historyTab === "image" ? (
                    <Icons.Image size={28} className="opacity-40" />
                  ) : (
                    <Icons.Video size={28} className="opacity-40" />
                  )}
                </div>
                <p className="text-sm font-medium">暂无生成历史</p>
                <p className={`text-xs mt-1 ${textMuted}`}>
                  生成的{historyTab === "image" ? "图片" : "视频"}将显示在这里
                </p>
              </div>
            ) : (
              <div className="grid grid-cols-2 gap-3">
                {(historyTab === "image" ? imageNodes : videoNodes).map(
                  (node) => (
                    <div
                      key={node.id}
                      ...
                      {historyTab === "image" ? (
                        ...
                        <div className="w-full h-full relative">
                          ...
                          <div className="absolute inset-0 flex items-center justify-center">
                            <div
                              className={`w-8 h-8 rounded-full flex items-center justify-center ${isDark ? "bg-white/20" : "bg-black/20"} backdrop-blur-sm`}
                            >
                              <Icons.Play
                                size={14}
          ...
          <div className={`px-4 py-3 border-t ${borderColor} shrink-0`}>
            ...
            >
              <span>
                共 {(historyTab === "image" ? imageNodes : videoNodes).length}{" "}
                项
              </span>
    ...
    return (
      ...
      >
        {/* Panel Header */}
        <div
          className={`px-4 py-3 border-b ${borderColor} flex items-center justify-between shrink-0`}
        >
  ...
  return (
    <>
      ...
      >
        ...
        {/* 第四行：排列工具（展开面板） */}
        <SidebarButton
          icon={Icons.AlignVerticalJustifyCenter}
          panel="TOOLS"
          tooltip="排列工具"
        />
