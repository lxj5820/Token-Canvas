  ...
  return (
    ...
    >
      ...
      {toolConfig.map((tool) => (
        ...
        >
          <span className={activeTool === tool.key ? "" : "opacity-60"}>
            {tool.icon}
          </span>
        </button>
      ))}
              ...
              {COLORS.map((color) => (
                <button
                  ...
                  className={`h-7 w-7 rounded-full border-2 transition-transform hover:scale-110 cursor-pointer shrink-0 ${
                    currentColor === color
                      ? "scale-110 border-white shadow-[0_0_6px_rgba(255,255,255,0.4)]"
                      : "border-white/10"
                  }`}
      ...
      <div className="inline-flex h-7 items-center justify-center gap-1.5 rounded-lg bg-transparent px-2 text-neutral-400">
        ...
        <div
          className="relative flex h-3 shrink-0 items-center"
