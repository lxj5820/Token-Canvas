    ...
    >
      {contextMenu.type === 'NODE' && contextMenu.nodeId && (() => {
        ...
        return (
          <>
            ...
            {contextMenu.nodeType === NodeType.ORIGINAL_IMAGE && onReplaceImage && (
              <button
                className={menuItemClass}
                onClick={() => {
            ...
            <button
              className={`text-left px-3 py-2 text-xs transition-all duration-150 flex items-center gap-2.5 rounded-md mx-1 text-red-400 ${
                isDark ? 'hover:bg-red-500/10 hover:text-red-300' : 'hover:bg-red-50 hover:text-red-600'
              }`}
              onClick={() => {
      ...
      {contextMenu.type === 'CANVAS' && (
        <>
          ...
          <div className={`h-px my-1.5 mx-2 ${isDark ? 'bg-zinc-700' : 'bg-gray-200'}`}></div>
          <div className={`px-3 py-1 text-[9px] font-semibold uppercase tracking-wider ${isDark ? 'text-zinc-500' : 'text-gray-400'}`}>
            添加节点
          </div>
          <button
            className={menuItemClass}
            onClick={() => {
              onAddNode(NodeType.TEXT_TO_IMAGE, contextMenu.worldX, contextMenu.worldY);
              onClose();
            }}
          >
          ...
          <button
            className={menuItemClass}
            onClick={() => {
              onAddNode(NodeType.TEXT_TO_VIDEO, contextMenu.worldX, contextMenu.worldY);
              onClose();
            }}
          >
            <div className="w-5 h-5 rounded bg-yellow-500/10 flex items-center justify-center">
          ...
          <button
            className={menuItemClass}
            onClick={() => {
              onAddNode(NodeType.TEXT_TO_AUDIO, contextMenu.worldX, contextMenu.worldY);
              onClose();
            }}
          >
          ...
          <button
            className={menuItemClass}
            onClick={() => {
              onAddNode(NodeType.START_END_TO_VIDEO, contextMenu.worldX, contextMenu.worldY);
              onClose();
            }}
          >
