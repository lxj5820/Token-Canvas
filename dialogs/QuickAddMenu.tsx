...
export const QuickAddMenu: React.FC<QuickAddMenuProps> = ({
  ...
  return (
    ...
    >
      <div
        className={`px-3 pb-2 mb-1 text-[11px] font-semibold border-b ${isDark ? "text-gray-200 border-zinc-800" : "text-gray-800 border-gray-100"}`}
      >
        连接到节点
      </div>
      ...
      <button
        ...
        onClick={() => onAddNode(NodeType.TEXT_TO_IMAGE)}
      >
      ...
      <button
        className={menuItemClass}
        onClick={() => onAddNode(NodeType.TEXT_TO_VIDEO)}
      >
        <div className="w-6 h-6 rounded-md bg-yellow-500/10 flex items-center justify-center">
      ...
      <button
        className={menuItemClass}
        onClick={() => onAddNode(NodeType.TEXT_TO_AUDIO)}
      >
      ...
      <button
        className={menuItemClass}
        onClick={() => onAddNode(NodeType.START_END_TO_VIDEO)}
      >
