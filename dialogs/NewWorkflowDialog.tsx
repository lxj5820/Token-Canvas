    ...
    >
      <div
        className={`w-[400px] p-6 rounded-2xl shadow-2xl border flex flex-col gap-4 transform transition-all scale-100 ${
          isDark ? 'bg-[#1A1D21] border-zinc-700 text-gray-200' : 'bg-white border-gray-200 text-gray-800'
        }`}
        ...
        <div className={`flex justify-end gap-2 mt-2 pt-4 border-t ${isDark ? 'border-zinc-800' : 'border-gray-100'}`}>
          ...
          <button
            ...
            className={`px-4 py-2 rounded-lg text-xs font-bold text-white transition-colors shadow-lg shadow-yellow-500/20 flex items-center gap-1.5 ${
              isDark ? 'bg-yellow-600 hover:bg-yellow-500' : 'bg-yellow-500 hover:bg-yellow-400'
            }`}
          >
            <Icons.Save size={14} />
            保存并新建
